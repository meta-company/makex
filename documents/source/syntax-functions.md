...
```{eval-rst}
.. py:function:: task(name, requires=None, inputs=None, steps=None, outputs=None, environment=None)
...
:::{todo}
TODO: SYNTAX_2025: fix the requires argument. 
:::
...
<!-- TODO: move this to types documentation-->
...
.. py:function:: home(*path:PathLike=None) -> Path
  ...
  :param path: Optional. A subpath in the home folder.
```

## source()

```{eval-rst}
.. py:function:: source(*path:PathLike=None) -> Path
  ...
  :param path: Optional. A subpath in the in the source folder.
```
